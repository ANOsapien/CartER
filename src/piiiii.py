...
class cart_pendulum():
        ...
        try:
            ...
            if(manual_continue):
                ...
                if self._auto_restart:
                    ...
                    # --- FIX IS HERE ---
                    # When an automated trial ends, increment the counter for the next trial.
                    self._current_pid_param_index += 1
    ...
    def pid(self):
        ...
        try:
            # Create a unique folder for each PID set run
            ...
            if self.flag_list["pid_input"]:
                ...
                if self.arduino.receive.rstrip() == "Kill switch hit.":
                    ...
                    if not self.temp_datum.flag_close_event:
                        ...
                        # Reconnect will increment the index and set the restart flag
                        self.reconnect(exp=True) 

                        # Now, check if all tests have been completed
                        if self._current_pid_param_index >= len(PID_PARAM_SETS):
                            print("All automated PID tests are complete. Exiting program.")
                            # Set a flag to ensure the main loop can break gracefully
                            self.flag_list["pid"] = False # Turn off pid mode
                            # We can also just exit here for simplicity in this script
                            exit() 
                        else:
                            # Otherwise, the main loop will handle the restart automatically
    ...
    def main(self, automate_setup=False, auto_restart=False):
        ...
        while True: # Outer loop for continuous operation of PID tests
            if self._should_restart_automation:
                # This condition will be true for the first run and for every subsequent restart
...
if(__name__ == "__main__"):
    ...
    # Assuming these are defined in external files as per the original code
    # If not, you need to define these classes
    # from data_process import data, live_data
    # from arduino_manager import arduino
    # from moment_data_process import data_frame
